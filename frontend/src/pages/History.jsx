...
export default function History() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URLS.BASE}/auth/history?token=${token}`).then((r) =>
        r.json(),
      ),
  ...
  const openTeacherDetail = async (session) => {
    const finishedGroups =
      session.groups?.filter((g) => g.game_state === "end_game") || [];
  ...
  const openDetail = async (game) => {
    ...
    try {
      const res = await fetch(
        `${API_URLS.BASE}/api/game/${game.group_id}/recap`,
      );
    ...
    <div className="relative flex flex-col items-center justify-center min-h-[calc(100vh-73px)] px-4 py-10">
            ...
            <div className="sticky top-0 glass-panel rounded-t-3xl px-6 py-4 border-b border-slate-700/50 flex items-center justify-between z-10">
              <div>
                <p className="font-bold text-white flex items-center gap-2">
                  ...
                  {MODE_LABELS[detailModal.game.game_mode] ||
                    detailModal.game.game_mode}
                ...
                <p className="text-slate-500 text-xs mt-0.5">
                  {detailModal.game.rounds_played} {t("history_rounds_pl")} ·{" "}
                  {detailModal.game.total_players} {t("end_players")}
                </p>
              </div>
              <button
                onClick={() => setDetailModal(null)}
              ...
              <div className="px-6 py-5 flex flex-col gap-5">
                  ...
                  {[
                    {
                      icon: "👥",
                      label: t("end_players"),
                      value: detailModal.recap.total_players,
                    },
                    {
                      icon: "🛡️",
                      label: t("end_survived"),
                      value: detailModal.recap.survivors,
                    },
                    {
                      icon: "☣️",
                      label: t("end_infection_rate"),
                      value: `${detailModal.recap.infection_rate}%`,
                    },
                ...
                <div className="bg-slate-800/60 rounded-2xl p-4 flex items-center gap-4">
                  <div className="text-4xl">
                    {detailModal.game.rank <= 3
                      ? RANK_EMOJI[detailModal.game.rank - 1]
                      : `#${detailModal.game.rank}`}
                  </div>
                  <div>
                    <p className="text-slate-400 text-xs uppercase tracking-widest">
                      {t("profile_your_result")}
                    </p>
                    <p className="text-white font-black text-xl">
                      {detailModal.game.score} {t("end_pts")}
                    </p>
                    <p className="text-slate-500 text-sm">
                      {detailModal.game.survived
                        ? `🛡️ ${t("end_survived")}`
                        : `🧟 ${t("end_infected")}`}{" "}
                      ·{t("profile_rank")} {detailModal.game.rank}/
                      {detailModal.game.total_players}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    {detailModal.recap.scoreboard?.map((player) => (
                      <div
                        key={player.username}
                        className={`flex items-center gap-3 p-3 rounded-2xl ${player.username === username ? "bg-[var(--neon-green-glow)]/10 border border-[var(--neon-cyan-glow)]/25" : "bg-slate-800/30"}`}
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm font-semibold truncate ${player.username === username ? "text-[var(--neon-cyan-glow)]/10" : "text-slate-200"}`}
                          >
                            {player.username}{" "}
          ...
          >
            <div className="sticky top-0 glass-panel rounded-t-3xl px-6 py-4 border-b border-slate-700/50 flex items-center justify-between z-10">
              <div>
                <p className="font-bold text-white flex items-center gap-2">
                  <BarChart2 size={16} className="text-cyan-400" />
                  {MODE_LABELS[teacherModal.session.game_mode] ||
                    teacherModal.session.game_mode}
                ...
                <p className="text-slate-500 text-xs mt-0.5">
                  {teacherModal.session.num_groups}{" "}
                  {teacherModal.session.num_groups === 1
                    ? t("dash_group").toLowerCase()
                    : `${t("dash_group").toLowerCase()}s`}{" "}
                  · {teacherModal.session.total_players}{" "}
                  {t("dash_students_label")}
                </p>
              </div>
              <button
                onClick={() => setTeacherModal(null)}
                      ...
                      {[
                        {
                          icon: "👥",
                          label: t("end_players"),
                          value: group.recap.total_players,
                        },
                        {
                          icon: "🛡️",
                          label: t("end_survived"),
                          value: group.recap.survivors,
                        },
                        {
                          icon: "☣️",
                          label: t("end_infection_rate"),
                          value: `${group.recap.infection_rate}%`,
                        },
                      ].map(({ icon, label, value }) => (
                        <div
                          key={label}
      ...
      <div className="flex items-center gap-3 mb-8 justify-center items-center">
        <div>
          ...
          <p className="text-[var(--neon-light-green-glow)] text-sm mt-0.5">
            {t("history_subtitle")}
          </p>
        </div>
      </div>
        ...
        <div className="flex gap-2 mb-6 bg-slate-800/60 p-1 rounded-2xl border border-slate-700/50">
          {hasStudent && (
            <button
              ...
              className={`flex-1 py-2.5 px-1 rounded-xl text-xs sm:text-sm font-bold transition-all flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 ${
                tab === "student"
        ...
        <div className="space-y-3">
          {recentGames.length === 0 && studentGames.length === 0 ? (
            <div className="glass-panel rounded-3xl p-10 text-center">
              ...
              <p className="text-slate-600 text-sm mt-1 mb-7">
                {t("history_join_session")}
              </p>
              <button
                onClick={() => navigate("/join")}
                className="mt-5 neon-btn-alt px-6 py-3 text-sm"
              >
                {t("history_join_game")}
              </button>
            </div>
          ) : (
            (recentGames.length > 0 ? recentGames : studentGames).map(
              (game, i) => {
                      ...
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <p className="text-slate-500 text-xs mt-0.5">
                            ...
                            {!isRecent &&
                              ` · ${t("history_group")} ${game.group_number}`}
                          </p>
                        </div>
                      </div>
                          ...
                          <span
                            className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                              finished
                                ...
                                : "bg-[var(--neon-green-glow)]/15 text-e[var(--neon-green-glow)]/80 border border-[var(--neon-green-glow)]/25"
                            }`}
                          >
                            {finished
                              ? t("history_finished")
      ...
      {tab === "teacher" && (
        <div className="space-y-3">
          {teacherGames.length === 0 ? (
            <div className="glass-panel rounded-3xl p-10 text-center">
              <p className="text-slate-400 font-semibold">
                {t("history_no_sessions")}
              </p>
              <p className="text-slate-600 text-sm mt-1">
                {t("history_create_session")}
              </p>
              <button
                onClick={() => navigate("/host")}
                className="mt-5 neon-btn-alt px-6 py-2.5 text-sm"
              >
                {t("history_host_game")}
              </button>
            </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div>
                        ...
                        <p className="text-slate-500 text-xs mt-0.5">
                          {session.num_groups}{" "}
                          {session.num_groups === 1
                            ? t("dash_group").toLowerCase()
                            : `${t("dash_group").toLowerCase()}s`}{" "}
                          · {session.total_players} {t("dash_students_label")}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span
                        className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                          session.status === "finished"
                            ? "bg-slate-700 text-slate-400"
                            : "bg-[var(--neon-green-glow)]/15 text-[var(--neon-green-glow)] border border-[var(--neon-green-glow)]/25"
                        }`}
                      >
                        {session.status === "finished"
                          ? t("history_finished")
