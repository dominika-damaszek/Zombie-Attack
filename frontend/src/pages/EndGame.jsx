    ...
    return (
      <div className="flex items-center justify-center min-h-[70vh]">
        <div className="text-center">
          ...
          <p className="text-slate-400 font-mono animate-pulse">Loading recap...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto py-8 px-4 animate-zw-fade">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-4xl font-black mb-2" style={{ color: '#AD9E97' }}>
          {infectedPct > 50 ? 'Zombies Win!' : 'Survivors Triumph!'}
        </h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Players', value: recap.total_players, icon: '👥', color: '#AD9E97' },
          { label: 'Survived', value: recap.survivors, icon: '🛡️', color: '#a8c4a0' },
          { label: 'Infected', value: recap.zombies, icon: '🧟', color: '#d97559' },
      ...
      <div className="glass-panel rounded-2xl p-5 mb-6">
        <div className="flex justify-between text-xs mb-2">
          <span className="flex items-center gap-1" style={{ color: '#a8c4a0' }}><Shield size={12} /> {survivedPct}% Survived</span>
          <span className="flex items-center gap-1" style={{ color: '#d97559' }}>{infectedPct}% Infected <Skull size={12} /></span>
        </div>
      ...
      {/* Zombie/Survivor Lists */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="glass-panel rounded-2xl p-4">
          <p className="text-xs uppercase tracking-widest mb-3 font-mono" style={{ color: '#a8c4a0' }}>🛡️ Survivors</p>
          {recap.survivor_names.length === 0 ? (
            <p className="text-slate-600 text-sm">None survived...</p>
          ) : recap.survivor_names.map((n) => (
            <p key={n} className="text-slate-300 text-sm py-1 border-b border-slate-700/40 last:border-0">{n}</p>
          ))}
        </div>
        <div className="glass-panel rounded-2xl p-4">
          <p className="text-xs uppercase tracking-widest mb-3 font-mono" style={{ color: '#d97559' }}>🧟 Zombies</p>
          {recap.zombie_names.length === 0 ? (
            <p className="text-slate-600 text-sm">No infections!</p>
      ...
      <div className="glass-panel rounded-2xl p-6 mb-6">
        <h2 className="font-bold text-lg mb-4 flex items-center gap-2" style={{ color: '#AD9E97' }}>
          <BookOpen size={20} /> What You Learned Today
        </h2>
