...
const ITEMS = [
  { label: 'Security Patch', emoji: '💊', color: 'text-rose-400',    bg: 'bg-rose-500/20 border-rose-500/30' },
  { label: 'System Boost',   emoji: '⚡', color: 'text-emerald-400', bg: 'bg-emerald-500/20 border-emerald-500/30' },
  { label: 'Firewall',       emoji: '🔥', color: 'text-orange-400',  bg: 'bg-orange-500/20 border-orange-500/30' },
  { label: 'Security Layer', emoji: '🛡️', color: 'text-blue-400',    bg: 'bg-blue-500/20 border-blue-500/30' },
  { label: 'Hacking Tool',   emoji: '🔓', color: 'text-yellow-400',  bg: 'bg-yellow-500/20 border-yellow-500/30' },
];
    ...
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="glass-panel rounded-2xl p-4 mb-5">
        ...
        <div className="flex flex-wrap gap-2">
          {ITEMS.map(item => (
            <span key={item.label} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold ${item.bg} ${item.color}`}>
      ...
      <div className="glass-panel rounded-3xl p-5 mb-5">
        ...
        <div
          id={READER_ID}
          className="w-full rounded-2xl overflow-hidden mb-4 transition-all"
          style={{
        ...
        {/* Flash feedback */}
        {lastFlash && (
          <div className={`text-center py-3 px-4 rounded-xl mb-3 font-semibold text-sm ${
            lastFlash.duplicate
        ...
        {scanning ? (
          <button
            ...
            className="w-full py-3 rounded-2xl font-bold flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 text-slate-200 transition-all"
          >
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl font-black flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white hover:from-emerald-400 hover:to-teal-400 transition-all text-lg"
          >
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
            {results.map((r, i) => (
              ...
              >
                ...
                <span className={`text-xs font-semibold ${r.known ? 'text-emerald-400' : 'text-rose-400'}`}>
