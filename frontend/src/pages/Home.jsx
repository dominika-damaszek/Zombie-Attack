...
const Home = ({ isAuthenticated }) => {
  ...
  const activeGame = (() => {
    try {
      return JSON.parse(localStorage.getItem("player_session") || "null");
    } catch {
      return null;
    }
  })();
  const hasActiveGame = !!(
    activeGame?.groupData?.group_id && activeGame?.playerData?.id
  );
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-57px)] sm:min-h-[calc(100vh-73px)] text-center px-4 py-8 relative">
      <div className="flex flex-col items-center gap-5 mb-8 sm:mb-14">
        ...
        <div>
          ...
          <p className="text-base sm:text-lg font-bold text-[var(--neon-pink)] tracking-widest uppercase mb-6">
            {t("home_slogan")}
          </p>
      ...
      <div className="flex flex-col gap-8 w-full lg:flex-row max-w-xl">
        ...
        >
          ...
          {t("home_join")}
          <span className="text-sm font-medium group-hover:font-bold -mt-1">
            Enter a classroom
          </span>
        </button>
        ...
        >
          ...
          {t("home_host")}
          <span className="text-sm font-medium text-[var(--neon-pink)]  group-hover:font-bold -mt-1">
      ...
      <div className="absolute bottom-[5%]  hidden lg:flex gap-10 flex-row w-[70%] justify-center">
        <div className="flex flex-row gap-3 items-center">
          <LazyImage
            src="/nrp1.png"
            alt="Players"
            className="w-[60px] h-[60px] object-contain drop-shadow-[0px_0_10px_rgba(255,255,255,0.3)]"
          />
